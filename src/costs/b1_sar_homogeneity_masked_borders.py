...
class masked_borders_B1SARHomogeneityCost(BaseCost):
    ...
    def calculate_cost(self, simulation_data: SimulationData, simulation, return_B1 = False, return_SAR = False) -> float:
        ...
        # Find borders
        borders_mask = find_borders(simulation_data)
        ...
        if return_B1 == False:
            return torch.mean(b1_field_subject_voxels) / torch.std(b1_field_subject_voxels) - lambda_SAR * b1_field_min / torch.sqrt(SAR_max)
        else:
            if return_SAR == False:
                return (torch.mean(b1_field_subject_voxels)/torch.std(b1_field_subject_voxels)) - lambda_SAR * SAR_max, b1_field_abs
            else:
                return (torch.mean(b1_field_subject_voxels)/torch.std(b1_field_subject_voxels)) - lambda_SAR * SAR_max, b1_field_abs, SAR_3D